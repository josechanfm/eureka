      ...
      <div class="container mx-auto pt-5">
        ...
        >
          ...
          <a-form-item :label="$t('project_duration')" name="duration">
            <a-input-number v-model:value="fund.duration" />{{ $t('phase') }}
          </a-form-item>
          ...
          <div class="flex flex-row item-center justify-center gap-5 pt-5">
            ...
            <a-button type="primary" html-type="submit" :disabled="fund.is_submitted"  v-role="['admin','dei']">
              <span v-if="fund.id">
                {{ $t('update') }}
              </span>
              <span v-else>
            ...
            <a-button :href="route('admin.fund.items.index',fund.id)" type="fund" v-if="fund.id">{{ $t('funding_items') }}</a-button>
          </div>
        </a-form>
        </div>
      </div>
